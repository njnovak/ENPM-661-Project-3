...
def expand_goal(color_map, goal_location, radius):
    ...
    for x_i in x_range:
        ...
        for y_i in y_range:
            if (x_i >= 0 and x_i < width) and (y_i >= 0 and y_i < height):
                if not (int(color_map[int(y_i)][int(x_i)][0]) == 255 or                       int(color_map[int(y_i)][int(x_i)][1]) == 255):
    ...
    for action in actions:
        ...
        if valid:
            for i in range(len(x_res)):
                if int(color_map[int(y_res[i])][int(x_res[i])][0]) == 255 and                   int(color_map[int(y_res[i])][int(x_res[i])][1]) == 0 and                   int(color_map[int(y_res[i])][int(x_res[i])][2]) == 0:
                    valid = False

            for i in range(len(x_res)):
                if int(color_map[int(y_res[i])][int(x_res[i])][0]) == 0 and                   int(color_map[int(y_res[i])][int(x_res[i])][1]) == 255 and                   int(color_map[int(y_res[i])][int(x_res[i])][2]) == 0:
                    valid = False
...
while len(open_nodes) > 0:
    ...
    if int(color_map[int(curr_y)][int(curr_x)][0]) == 0 and       int(color_map[int(curr_y)][int(curr_x)][1]) == 0 and       int(color_map[int(curr_y)][int(curr_x)][2]) == 255:
...
def calc_vels(node):
    ...
    L_T = 0.35
    count = 0
    rospy.init_node('project3_p2')
    cmd_vel = rospy.Publisher('cmd_vel', Twist, queue_size=10)

    # sub2 = rospy.Subscriber("/odom",Odometry,move_robot)
    # if count == 0:
    #     # rospy.init_node("/cmd_vel")
    #     move_cmd = Twist()
    #     move_cmd.linear.x = 0.5
    #     move_cmd.angular.z = 0
    #     # while x != node.cell_location[0] and y != node.cell_location[1]:
    #     # rospy.loginfo("Moving forward at ",move_cmd," velocities")
    ...
    #     # cmd_vel.publish(move_cmd)
    #     count += 1
    if node.command[0] == node.command[1]:
        # cmd_vel = rospy.Publisher('cmd_vel', Twist, queue_size=10)
        move_cmd = Twist()
        dot_x = 20*(RAD_T/2)*(node.command[0]/20+node.command[1]/20)*abs(math.cos(node.cell_location[2]))
        move_cmd.angular.z = 0
        # while x != node.cell_location[0] and y != node.cell_location[1]:
        rospy.loginfo("Moving forward at " +str(dot_x))
        cmd_vel.publish(move_cmd)
        rospy.sleep(1)
        move_cmd.linear.x = 0
        ...
        # dot_x = 1.24
        dot_x = 20*(RAD_T/2)*(node.command[0]/20+node.command[1]/20)*abs(math.cos(node.cell_location[2]))
        # dot_y = (RAD_T/2)*(node.command[0]+node.command[1])*math.sin(node.cell_location[2])
        # dot_x = 1
        # dot_y = 1*abs(math.sin(node.cell_location[2]))
        dot_theta =20*(RAD_T/L_T)*(node.command[0]/20-node.command[1]/20)
        rospy.loginfo("Turning at "+str(dot_x)+" forward and "+str(dot_theta)+" turning")
        move_cmd = Twist()
        move_cmd.linear.x = dot_x
        # move_cmd.linear.y = dot_y
        move_cmd.angular.z = dot_theta
        # while x != node.cell_location[0] and y != node.cell_location[1]:
        # rospy.loginfo("Moving forward and turning at ",move_cmd," velocities")
        cmd_vel.publish(move_cmd)
        rospy.sleep(1)
        move_cmd.linear.x = 0
        # move_cmd.linear.y = 0
        ...
        cmd_vel.publish(move_cmd)
        rospy.sleep(1)
# with open("/home/nick/catkin_ws/src/project3_p2/src/sol_path.txt",'r') as all_paths:
    # print("Here")
def stop_bot():
    cmd_vel = rospy.Publisher('cmd_vel', Twist, queue_size=10)
    move_cmd = Twist()
...
#     calc_vl
class Node2:
    def __init__(self,cell_location,command):
        self.cell_location = cell_location
        self.command = command
count = 0
for node in solution_path:
    # node = Node2(node[0],node[1])
    if count == 0:
        count+=1
        
    else:
        print(node.cell_location,node.command)
        calc_vels(node)
        rospy.loginfo("Moving to location: "+str(node.cell_location[0]/20)+" "+str(node.cell_location[1]/20)+" "+str(node.cell_location[2])+"with command "+str(node.command))
rospy.loginfo("Success!")
stop_bot()

    # print(str([node.cell_location,node.command]))
# Wrong MAP
...
#     # else:
#         # dot_x = 1.24
#     dot_x = (RAD_T/2)*(node.command[0]+node.command[1])*abs(math.cos(node.cell_location[2]))
#     # dot_y = (RAD_T/2)*(node.command[0]+node.command[1])*math.sin(node.cell_location[2])
#     dot_x = 1
#     # dot_y = 1*abs(math.sin(node.cell_location[2]))
#     dot_theta =(RAD_T/L_T)*(node.command[1]-node.command[0])
